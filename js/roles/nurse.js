...
async function nurse_intake(appointmentId = null) {
  ...
  const details = appointmentId ? await api(`api/nurse/appointment_get.php?appointmentId=${appointmentId}`) : null;
  ...
  if (appointmentId) nurse_renderIntakeForm(appointmentId);
}

async function nurse_renderIntakeForm(appointmentId) {
  const wrap = document.getElementById("nurse_intake_form");
  if (!wrap) return;
  const existing = await api(`api/nurse/intake_get.php?appointmentId=${appointmentId}`).catch(() => ({ intake: null }));
